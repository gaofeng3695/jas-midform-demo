...
Vue.component('jas-dialog-wrapper', {
	...
	template: [
		'<div class="jas-dialog-wrapper jas-flex-box is-vertical">',
		'  <div class="is-grown" style="padding: 0 20px;overflow: auto;">',
		'    <div><slot></slot></div>',
		'  </div>',
		'  <div class="jas-dialog-wrapper-footer" style="text-align: center;padding-top:10px;margin: 0 20px;border-top: 1px solid #e4e7ed;">',
...
Vue.component('jas-file-list-new', {
	...
	template: [
		'<div class="jas-file-list-new">',
		'  <div class="jas-file-list-new-tip" v-show="fileList.length == 0">暂无信息</div>',
...
Vue.component('jas-pic-list', {
	...
	template: [
		'<div class="jas-pic-list" style="padding-bottom: 10px;">',
		'  <div class="jas-pic-list-tip" v-show="fileList.length == 0">暂无图片</div>',
...
Vue.component('jas-file-upload-new', {
	...
	methods: {
		insertIcon: function (file) {
			...
			this.$nextTick(function () {
				...
				$(that.$refs.upload.$el).find('li').each(function (i) {
					var oPic = that.$refs.upload.uploadFiles[i];
					var fileId = oPic.oid ? oPic.oid : jasTools.base.createuuid();
					if ($(this).find('img').length) { // 图片
						if ($(this).find('.picName').length) return;
						if (oPic.status === "ready") {
							that.$refs.upload.uploadFiles[i].fileId = fileId;
							that.fileIdList.push({
								fileId: fileId,
								businessType: "attachment",
								fileType: that.fileType,
							});
						}
						var $dom = $('<span style="display:none;" id="file_id_' + (oPic.oid || oPic.uid) + '" >' + fileId + '</span><div class="picName" style="color: #fff;background: rgba(0,0,0,0.3);overflow: hidden;text-overflow: ellipsis;width:146px;height: 28px;line-height: 28px;box-sizing:border-box;position:absolute;bottom:0;right:0;padding:0px 10px;">' + oPic.name + '</div>');
						// $dom.val(oPic.name)
						...
						var $input = $('<span style="display:none;" id="file_id_' + (oPic.oid || oPic.uid) + '">' + fileId + '</span><input id="' + (oPic.oid || oPic.uid) + '" data-oldval="' + (oPic.fileDescription || '') + '" class="el-input__inner" placeholder="请输入备注" maxlength="150" style="width: 30%;height: 28px;line-height: 28px;"></input>');
						$input.on('keydown', function (e) {
							if (e.keyCode == 8) {
								e.stopPropagation && e.stopPropagation();
							}
						}).val(oPic.fileDescription).on('change', function () {
							if ($(this).val().length > 199) {
								top.Vue.prototype.$message.error('文件备注不得超过' + 200 + '字');
							}
						});
						$(this).css({
							padding: '4px'
		...
		beforeUpload: function (file) {
			...
			var fileId = $('#file_id_' + file.uid).html();
			this.formData.fileDescription = desc;
			this.formData.fileId = fileId;
			// file.fileDescription = 'asdasd'
			var fileIndexNum = this.$refs.upload.uploadFiles.findIndex(function (item) {
				return item.name == file.name;
			});
			this.formData.fileIndexNum = fileIndexNum;
		},
		...
		requestFile: function (bizId, businessType) {
			...
			jasTools.ajax.get(url, {
				...
				data.rows.forEach(function (item) {
					...
					var aName = item.fileName.split('.');
					var sType = aName[aName.length - 1].toLowerCase();
					if (['bmp', 'tif', 'jpg', 'png', 'gif'].indexOf(sType) > -1) {
						// if (that.fileType == 'pic') {
						var url = jasTools.base.rootPath + "/attachment/getImageBySize.do";
						file.url = jasTools.base.setParamsToUrl(url, {
		...
		removeFile: function (file, fileList) {
			if (file.status === 'success' && file.oid) {
				...
				this.fileIdList = this.fileIdList.filter(function (item, index) {
		...
		uploadFile: function (oid, bizType) {
			var that = this;
			var files = that.$refs.upload.uploadFiles;
			for (var i = 0; i < files.length; i++) {
				console.log(files[i].fileDescription)
				if (files[i].fileDescription && (files[i].fileDescription.length > 199)) {
					top.Vue.prototype.$message.error('文件备注不得超过' + 200 + '字');
					return;
				}
			}
...
Vue.component('jas-import-export-btns', {
	...
	methods: {
		...
		bt_export: function (obj) {
			var param = window.jasTools.base.getParamsInUrl(location.href);
			var parentObj = param.parentObj ? JSON.parse(decodeURI(param.parentObj)) : "";
			...
			jasTools.ajax.post(url, {
				...
				keywords: {
					...
					parentId: parentObj.oid || parentObj.OID,
				}
			}, function (data) {
		...
		bt_export_all: function (obj) { // 导出全部
			...
			this.form.parentId = parentObj.oid || parentObj.OID,
			jasTools.ajax.post(url, {
				templateCode: this.exportTemplateCode,
...
Vue.component('jas-form-items-group', {
	...
	template: [
		...
		'		<template v-if="fieldsConfig[item.field].type == \'textarea\'">',
		'			<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">',
		'				<jas-textarea :title="item.name" :maxLength="fieldsConfig[item.field].maxLength || 200" v-model="form[item.field]" :prop="item.field" :rules="fieldsConfig[item.field] && fieldsConfig[item.field].rules"></jas-textarea>',
		'			</el-col>',
		'		</template>',
		'		<template v-else-if="fieldsConfig[item.field].type == \'mediuminput\'">',
		'			<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">',
...
Vue.component('jas-approve-dialog', {
	...
	watch: {
		childPageUrl: function (newval) {
			var that = this;
			if (newval) {
				this.showSecond = !this.showSecond;
				setTimeout(function () {
					that.showSecond = !that.showSecond;
				});
			}
		}
	},
	template: [
		...
		'			</div>',
		'  </el-tab-pane>',
		'  <el-tab-pane v-if="childPageUrl" label="扩展信息" name="middle">',
		'    <div style="height: 100%;overflow: auto;">',
		'			<iframe  :src="childPageUrl" style="height:calc( 100% - 7px );width:100%;" frameborder="0"></iframe>',
		'			</div>',
		'  </el-tab-pane>',
		'  <el-tab-pane v-if="showSecond" label="审核信息" name="second">',
