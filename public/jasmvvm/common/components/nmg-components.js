...
Vue.component('nmg-county-cascader', { //value 值 支持逗号分隔 的多选下拉框
  ...
  computed: {
    _value: {
      ...
      set: function (newValue) {
        // this.cLabel = (this.$refs.elcasc.inputValue || '').split(' / ');
  ...
  methods: {
    handelArea: function (item) {
      ...
      setTimeout(function () {
        that.cLabel = (that.$refs.elcasc.inputValue || '').split(' / ');
      }, 100);
    },
    ...
    requestAreaData: function () {
      ...
      jasTools.ajax.post(jasTools.base.rootPath + "/nmpsp/area/getCityAndCounty.do", {
        "functionCode": menuCode
      }, function (data) {
        if (data.status === 1) {
