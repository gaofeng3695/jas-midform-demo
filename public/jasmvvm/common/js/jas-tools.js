 ...
 *------  jasTools.base.switchToLowerCaseAndCamelCase 大写转小写，下划线转驼峰
...
(function (window) {
	...
	var tools = (function () {
		...
		var switchToLowerCaseAndCamelCase = function (string) {
			// Support: IE9-11+
		...
		return {
			...
			switchToLowerCaseAndCamelCase: switchToLowerCaseAndCamelCase,
	...
	var ajax = (function () {
		var ajax = function (type, url, params, cb_success, cb_fail, isSync) {
			...
			$.ajax({
				...
				data: data,
				async: !isSync ? true : false,//使用同步的方式,true为异步方式
				success: function (data, status) {
	...
	var vScroll = (function () {
		...
		var init = function (options) {
			seeped = options.seeped;
			$el = options.el;
			$($el).each(function (i) {
				intId[i] = createSet($($el));
				$($el).hover(function () {
					clearAll();
				}, function () {
					if (intId[0]) {
		...
		var marquee = function (obj) {
			obj.find(".marquee").animate({
				marginTop: '-=1'
			}, 0, function () {
				var s = Math.abs(parseInt($(this).css("margin-top")));
				var node = $(this).context.childNodes[0].nodeName;
				if (s >= $(this).find(node)[0].offsetHeight) {
					$(this).find(node).slice(0, 1).appendTo($(this));
					$(this).css("margin-top", 0);
				}
			});
		};
		var clearAll = function (cb) {
			intId.forEach(function (item) {
				clearInterval(item);
			});
			if (cb) {
				cb();
	 ...
	 * 弹出详情信息相关操作
	 */
	var selfDialogNew = (function (tools) {
		...
		var show = function (params) {
			...
			var html = [
				'<div class="new-mask"><jas-dialog-new',
				'  :title="title" ',
				'  :iframeUrl="iframeUrl" ',
				'  :height="height" ',
				'  :width="width" ',
