...
window.app = new Vue({
	...
	methods: {
		_isGoPage: function () {
			var that = this;
			//进行登录人员所属地区的查询
			var url = jasTools.base.rootPath + '/nmpsp/basedatastatistics/queryLoginUserArea.do';

			jasTools.ajax.get(url, {}, function (data) {

				if (data.status == 1) {
					if (data.data.areaType) {
						that.isGoPage = true;
					}
					that.isUserInfo = data.data;
				}
			});
		},
		_goPage: function () {
			var that = this;
			var url = jasTools.base.rootPath + '/jasmvvm/pages/group-onepage-new/basic/basic.html?areaType=' + that.isUserInfo.areaType + "&areaId=" + that.isUserInfo.areaId;
			window.open(url)
			// location.href = jasTools.base.rootPath + '/jasmvvm/pages/group-onepage-new/basic/basic.html?areaType=' + that.isUserInfo.areaType + "&areaId=" + that.isUserInfo.areaId;
		},
		backToDesk: function () {
		...
		selectMenu: function (index) {
			...
			this.changeLayersVisiable();
		},
		changeLayersVisiable: function () {
			var that = this;
			var baseLayer = ["BASE_STATION", "BASE_VALVE_ROOM", "BASE_PIPELINE", "BASE_MARKER", "BASE_PIPELINE_NAME", "BIZ_HCA_POLYGON", "BASE_PIPELINE_SEGMENT"];
			var layers = JSON.parse(localStorage.getItem("allLayers"));

			layers.forEach(function (layerId) {

				if (baseLayer.includes(layerId)) {

					top.frames['gisWindow'].jasMap.layerVisibleSwitch(layerId, true);
				} else {
					top.frames['gisWindow'].jasMap.layerVisibleSwitch(layerId, false);
				}
			});
		},
		...
		_showPersonInfo: function () {
			...
			jasTools.dialog.show({
				...
				width: '530px',
				height: '380px',
				src: '../page-person/personInfo.html',
				cbForClose: function (param) {
		...
		locate: function (oid, tableName) {
			this.$refs.resizer.panelShowed = true;
			var options = {
				scale: 100,
				expand: 10,
			}
			top.frames['gisWindow'].jasMap.flashGraphic(oid, tableName, options);
			// if (tableName == "BIZ_HCA") {
			// 	top.frames['gisWindow'].jasMap.flashGraphic(oid, "BIZ_HCA_polygon", options);
			// }
			// if (tableName == "BIZ_MAJOR_WORK_SITE") {
			// 	top.frames['gisWindow'].jasMap.flashGraphic(oid, "BIZ_MAJOR_WORK_SITE_point", options);
			// }
			// if (tableName == "BIZ_GEOLOGIC_HAZARD") {
			// 	top.frames['gisWindow'].jasMap.flashGraphic(oid, "BIZ_GEOLOGIC_HAZARD_point", options);
			// }
		},
		_queryTodoNum: function () {
			...
			var that = this;
			//获取数量
			var url = jasTools.base.rootPath + '/jasframework/workflow/task/myTodoList.do';
			jasTools.ajax.post(url, {}, function (data) {
				if (data.status == 1) {
					that.todoNum = data.total;
				}
			}, function () {
