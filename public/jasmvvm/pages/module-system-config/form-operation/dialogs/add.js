...
var vm = new Vue({
  ...
  methods: {
    ...
    isJsonSelect: function (type) {
      return type === 'UT_12' && this.isUi(type);
    },
    ...
    isUrl: function (row) {
      var type = row.uiType;
      var isShow = (this.isSqlSelect(type) || this.isJsonSelect(type)) && row.childFieldArr && row.childFieldArr.length > 0;
      if (!isShow) {
      ...
      if (ifHasSortable) {
        ...
        this.rows.sort(function (a, b) {
          if (a.list[0] && b.list[0]) {
            return a.list[0].groupIndex - b.list[0].groupIndex
          }
        });
        debugger
      } else {
