<template>
  <!-- header 开始 -->
  <div class="header headerwrapper">
    <div class="logo item">
    </div>
    <div class="name item">
      <!-- 中盈安信基础服务一体化平台 -->
...
export default {
  data() {
    return {
      ...
      todoNum: 0,
      username:
        this.$jasStorage.get("user") &&
        JSON.parse(this.$jasStorage.get("user")).userName,
      userunit:
        this.$jasStorage.get("user") &&
        JSON.parse(this.$jasStorage.get("user")).unitName,
    };
  },
  methods: {
    handleCommand() {},
